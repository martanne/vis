...
lex:add_rule('keyword', token(lexer.KEYWORD, word_match[[
  ...
  default.target display-manager.service emergency.target exit.target
  final.target getty.target graphical.target hibernate.target
  hybrid-sleep.target halt.target initrd-fs.target kbrequest.target kexec.target
  local-fs.target multi-user.target network-online.target paths.target
  poweroff.target reboot.target remote-fs.target rescue.target
  initrd-root-fs.target runlevel2.target runlevel3.target runlevel4.target
  runlevel5.target shutdown.target sigpwr.target sleep.target slices.target
  sockets.target suspend.target swap.target sysinit.target syslog.socket
  system-update.target timers.target umount.target
  ...
  PATH LANG USER LOGNAME HOME SHELL XDG_RUNTIME_DIR XDG_SESSION_ID XDG_SEAT
  XDG_VTNR MAINPID MANAGERPID LISTEN_FDS LISTEN_PID LISTEN_FDNAMES NOTIFY_SOCKET
  WATCHDOG_PID WATCHDOG_USEC TERM
]]))

-- Options.
lex:add_rule('option', token(lexer.PREPROCESSOR, word_match[[
  -- Unit section.
  Description Documentation Requires Requisite Wants BindsTo PartOf Conflicts
  Before After OnFailure PropagatesReloadTo ReloadPropagatedFrom
  ...
  ConditionACPower ConditionNeedsUpdate ConditionFirstBoot ConditionPathExists
  ...
  ConditionFileNotEmpty ConditionFileIsExecutable AssertArchitecture
  AssertVirtualization AssertHost AssertKernelCommandLine AssertSecurity
  ...
  AssertDirectoryNotEmpty AssertFileNotEmpty AssertFileIsExecutable SourcePath
  -- Install section.
  ...
  PermissionsStartOnly RootDirectoryStartOnly NonBlocking NotifyAccess Sockets
  FailureAction FileDescriptorStoreMax USBFunctionDescriptors USBFunctionStrings
  -- Socket section.
  ...
  KeepAliveProbes NoDelay Priority DeferAcceptSec ReceiveBuffer SendBuffer IPTOS
  IPTTL Mark ReusePort SmackLabel SmackLabelIPIn SmackLabelIPOut
  ...
  OnCalendar AccuracySec RandomizedDelaySec Unit Persistent WakeSystem
  RemainAfterElapse
  -- Exec section.
...
lex:add_rule('section', token(lexer.LABEL, '[' * word_match[[
  Automount BusName Install Mount Path Service Service Socket Timer Unit
]] * ']'))
