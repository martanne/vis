...
lex:add_rule('keyword', token(lexer.KEYWORD, word_match[[
  ...
  define-values delay delay-force do if guard lambda let let* let*-values
  let-syntax let-values letrec letrec* letrec-syntax parameterize quasiquote
...
lex:add_rule('function', token(lexer.FUNCTION, word_match[[
  * + - / < <= = => > >= abs append apply assoc assq assv binary-port? boolean=?
  ...
  cdar cddr cdr ceiling char->integer char-ready? char<=? char<? char=? char>=?
  char>? char? close-input-port close-output-port close-port complex? cons
  ...
  exact-integer-sqrt exact-integer? exact? expt features file-error? floor
  floor-quotient floor-remainder floor/ flush-output-port for-each gcd
  get-output-bytevector get-output-string inexact inexact? input-port-open?
  input-port? integer->char integer? lcm length list list->string list->vector
  ...
  negative? newline null? number->string number? numerator odd?
  open-input-bytevector open-input-string open-output-bytevector
  open-output-string output-port-open? output-port? pair? peek-char peek-u8
  port? positive? procedure? quotient raise raise-continuable rational?
  rationalize read-bytevector read-bytevector! read-char read-error? read-line
  read-string read-u8 real? remainder reverse round set-car! set-cdr! square
  ...
  truncate-remainder truncate/ u8-ready? utf8->string values vector vector->list
...
-- Directives.
lex:add_rule('directive', token(lexer.PREPROCESSOR, P('#!fold-case') +
  '#!no-fold-case'))
...
-- Numbers.
local radixes = {
  [2] = P('#b'), [8] = P('#o'), [10] = P('#d')^-1, [16] = P('#x')
}
local digits = {
  [2] = S('01'), [8] = lpeg.R('07'), [10] = lexer.digit, [16] = lexer.xdigit
}
